...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    const detectBackground = () => {
      // Obtener el elemento detrás del navbar
      ...
      // Ocultar temporalmente el navbar para detectar lo que hay detrás
      ...
      if (elementBehind) {
        ...
        // Si hay imagen de fondo, asumimos fondo oscuro por defecto
        ...
        if (rgb && rgb.length >= 3) {
          ...
          // Si la luminosidad es mayor a 0.5, es fondo claro
  ...
  return (
    <nav
      className="fixed bottom-0 inset-x-0 sm:sticky sm:top-0 shadow-lg z-[200] transition-colors duration-300"
      style={{
        background: isDarkBackground
          ? `rgba(244, 244, 244, ${0.15 * navOpacity})`
          : `rgba(0, 0, 0, ${0.92 * navOpacity})`,
        backdropFilter: "blur(20px)",
        WebkitBackdropFilter: "blur(20px)",
        borderBottom: isDarkBackground
          ? `1px solid rgba(244, 244, 244, ${0.3 * navOpacity})`
          : `1px solid rgba(255, 255, 255, ${0.25 * navOpacity})`,
      }}
    >
      <div className="w-full px-4 sm:px-8 lg:px-10">
        <div className="hidden sm:flex items-center justify-between h-16 gap-8 sm:gap-12">
          ...
          <div className="flex-1 flex items-center justify-center gap-3">
            ...
            <div
              className={`relative hidden sm:flex items-center glass-strong rounded-full shadow-md transition-all duration-300 ${
          ...
          <div className="flex items-center justify-end min-w-[200px]">
            {currentUser && !isGuest ? (
              <div className="relative flex items-center gap-2" ref={menuRef}>
                ...
                <div
                  className={`absolute right-0 sm:top-full sm:mt-2 sm:origin-top bottom-full mb-2 origin-bottom w-52 border rounded-xl shadow-2xl z-[300] transition-all duration-200 ease-out transform ${
                    menuOpen
                      ? "opacity-100 translate-y-0 scale-100 pointer-events-auto"
                      : "opacity-0 sm:-translate-y-2 translate-y-2 scale-95 pointer-events-none"
                  }`}
                  style={{
                ...
                >
                  <div className="px-4 py-3 text-base text-white font-bold border-b border-white/30 mb-1 bg-black/40">
                    {displayName}
                  </div>
                  <div className="p-2">
                    <button
                      className="w-full text-left text-white hover:bg-white/20 px-4 py-3 rounded-lg mb-1 transition-all font-medium text-sm flex items-center gap-2 bg-zinc-800/50"
                      onClick={() => {
                        setMenuOpen(false);
                        navigate("/profile");
                      }}
                    ...
                    <button
                      className="w-full text-left text-white hover:bg-red-600/50 px-4 py-3 rounded-lg transition-all font-medium text-sm flex items-center gap-2 bg-zinc-800/50"
