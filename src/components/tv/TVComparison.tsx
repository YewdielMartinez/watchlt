...
const TVComparison: React.FC<Props> = ({ shows }) => {
    ...
    try {
      ...
      if (saved) {
        ...
        const ok =
          ...
          keys.every((k) => parsed.find((p) => p.key === k));
        if (ok) return parsed;
      }
    } catch {}
    return [
  ...
  useEffect(() => {
    const fetchMissing = async () => {
      for (const s of shows) {
        const needSeasons =
          !s.number_of_seasons && !detailsCache[s.id]?.number_of_seasons;
        const needEpisodes =
          !s.number_of_episodes && !detailsCache[s.id]?.number_of_episodes;
        const needRuntime =
          !(s.episode_run_time && s.episode_run_time.length) &&
          !(
            detailsCache[s.id]?.episode_run_time &&
            detailsCache[s.id]?.episode_run_time?.length
          );
  ...
  useEffect(() => {
    try {
      localStorage.setItem("tvCriteriaDefaults", JSON.stringify(criteria));
    } catch {}
  }, [criteria]);
  ...
  const calcScores = () => {
    ...
    const seasons = shows.map(
      (s) => s.number_of_seasons ?? detailsCache[s.id]?.number_of_seasons ?? 0
    );
    const episodes = shows.map(
      (s) => s.number_of_episodes ?? detailsCache[s.id]?.number_of_episodes ?? 0
    );
    const epRuntime = shows.map(
      (s) =>
        s.episode_run_time?.[0] ??
        detailsCache[s.id]?.episode_run_time?.[0] ??
        0
    );
    ...
    const minSeasons = Math.min(...seasons);
    const maxSeasons = Math.max(...seasons);
    const seasonsRange = Math.max(1, maxSeasons - minSeasons);
    const minEpisodes = Math.min(...episodes);
    const maxEpisodes = Math.max(...episodes);
    const episodesRange = Math.max(1, maxEpisodes - minEpisodes);
    const minRuntime = Math.min(...epRuntime);
    const maxRuntime = Math.max(...epRuntime);
    const runtimeRange = Math.max(1, maxRuntime - minRuntime);
    ...
    return shows
      .map((show) => {
        ...
        const yearScore10 = Math.min(
          10,
          Math.max(0, ((startYear - 1990) / (currentYear - 1990)) * 10)
        );
        const popularityScore10 =
          (((show.popularity ?? 0) - minPop) / popRange) * 10;
        const votesScore10 =
          (((show.vote_count ?? 0) - minVotes) / votesRange) * 10;
        const seasonsVal =
          show.number_of_seasons ??
          detailsCache[show.id]?.number_of_seasons ??
          0;
        const seasonsScore10 = ((seasonsVal - minSeasons) / seasonsRange) * 10;
        const episodesVal =
          show.number_of_episodes ??
          detailsCache[show.id]?.number_of_episodes ??
          0;
        const episodesScore10 =
          ((episodesVal - minEpisodes) / episodesRange) * 10;
        const runtimeVal =
          show.episode_run_time?.[0] ??
          detailsCache[show.id]?.episode_run_time?.[0] ??
          0;
  ...
  const chartData = {
    ...
    datasets: scores.map((score, index) => ({
      label: score.show.name,
      data: [
        score.rating10,
        score.yearScore10,
        score.popularityScore10,
        score.votesScore10,
        score.seasonsVal / 10, // Normalizar a escala 0-10
        score.episodesVal / 50, // Normalizar a escala 0-10 (asumiendo max ~500 episodios)
        score.runtimeVal / 10, // Normalizar a escala 0-10 (asumiendo max ~100 min)
      ],
      backgroundColor: radarColors[index % radarColors.length].bg,
      borderColor: radarColors[index % radarColors.length].border,
      borderWidth: 2,
      pointBackgroundColor: radarColors[index % radarColors.length].border,
      pointBorderColor: "#fff",
      pointHoverBackgroundColor: "#fff",
      pointHoverBorderColor: radarColors[index % radarColors.length].border,
    })),
  };
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "Comparación multidimensional de series",
        color: "#ffffff",
        font: {
          size: 16,
    ...
    <div>
      {!showAll && top && (
        <div className="mb-8 flex flex-col items-center text-center">
          <img
            src={
              top.show.poster_path
                ? `https://image.tmdb.org/t/p/w500${top.show.poster_path}`
                : "https://via.placeholder.com/500x750?text=No+Image"
            }
            alt={top.show.name}
            className="w-40 sm:w-56 md:w-64 lg:w-72 rounded-lg shadow-xl object-cover mb-3"
          />
          <h3 className="text-2xl font-semibold text-tertiary mb-1">
            {top.show.name}
          </h3>
          <p className="text-tertiary/80 mb-3">
            {top.show.first_air_date
              ? new Date(top.show.first_air_date).getFullYear()
              : "—"}{" "}
            • {top.seasonsVal} temp • {top.episodesVal} ep •{" "}
            {typeof top.show.vote_average === "number"
              ? top.show.vote_average.toFixed(1)
              : "—"}
            ⭐
          </p>
          <div className="flex items-center gap-4">
            <div className="text-sm text-tertiary/80">
              Mejor opción según tus criterios
            </div>
            <div className="text-4xl font-bold text-tertiary">
              {top.weighted100} pts
            </div>
            <button className="btn-primary" onClick={openDetails}>
              Ver más
            </button>
          </div>
        </div>
      ...
      {showAll && (
        <div className="mb-6" id="comparacion-detalle">
          <h3 className="text-lg font-medium text-tertiary mb-3">
            Ajustar criterios de comparación
          </h3>
            ...
            {criteria.map((c, i) => (
              <div key={c.key} className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-tertiary/90">
                    Importancia:{" "}
                    <span className="font-semibold">{c.weight}/10</span>
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    value={c.weight}
                    onChange={(e) =>
                      updateCriteriaWeight(i, parseInt(e.target.value))
                    }
      ...
      {showAll && showInfo && (
        <div className="mb-6 p-3 rounded-lg border border-primary/30 bg-primary/20 text-tertiary text-[13px] leading-relaxed">
          <p className="opacity-90">
            El puntaje final (1–100) combina métricas de cada serie normalizadas
            entre 0–10 y ponderadas por la importancia elegida.
          </p>
          <ul className="list-disc pl-5 mt-2 space-y-1 opacity-90">
            ...
            <li>
              Popularidad, votos, temporadas, episodios y duración de episodio
              se normalizan entre el mínimo y máximo del conjunto.
            </li>
            <li>
              Puntaje final = suma( peso_i × métrica_i_normalizada ) /
              suma(pesos) × 10.
            </li>
            <li>
              Ordenamos de mayor a menor; la primera fila es la más compatible.
            </li>
          </ul>
        </div>
      )}
        ...
        <div className="overflow-x-auto card">
          <table className="min-w-full divide-y divide-primary/40 border border-primary border-opacity-30">
            <thead className="bg-primary bg-opacity-25">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-tertiary opacity-80 uppercase tracking-wider">
                  Serie
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-tertiary opacity-80 uppercase tracking-wider">
                  Calificación
            ...
            <tbody className="bg-primary bg-opacity-10 divide-y divide-primary/30">
              {(showAll ? scores : scores.slice(0, 1)).map((s) => (
                <tr
                  key={s.show.id}
                  className={s === scores[0] ? "bg-accent bg-opacity-10" : ""}
                >
                    ...
                    <div className="flex items-center">
                      <div className="flex-shrink-0 h-10 w-10">
                        <img
                          ...
                          src={
                            s.show.poster_path
                              ? `https://image.tmdb.org/t/p/w92${s.show.poster_path}`
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-tertiary">
                      {(s.show.vote_average ?? 0).toFixed(1)}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-tertiary">
                      {s.show.first_air_date
                        ? new Date(s.show.first_air_date).getFullYear()
                        : "N/A"}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-tertiary">
                      {(s.show.popularity ?? 0).toFixed(0)}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-tertiary">
                      {(s.show.vote_count ?? 0).toLocaleString()}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-tertiary">{s.seasonsVal}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-tertiary">{s.episodesVal}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-tertiary">
                      {s.runtimeVal} min
