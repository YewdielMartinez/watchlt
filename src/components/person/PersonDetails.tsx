...
const PersonDetails: React.FC = () => {
  ...
  const knownFor = useMemo(() => {
    ...
    const pool = (credits.cast || []).filter(
      (c) =>
        (c.media_type === "movie" || c.media_type === "tv") && c.poster_path
    );
    return pool
      .slice()
      .sort(
                    ...
                    <dd>
                      ...
                      {person.birthday && (
                        <span className="opacity-75">
                          ({ageFrom(person.birthday, person.deathday)})
                        </span>
                      )}
                    </dd>
                  </div>
                  ...
                  {person.also_known_as && person.also_known_as.length > 0 && (
                    <div>
                      ...
                      <dd className="whitespace-pre-line">
                        {person.also_known_as.slice(0, 6).join("\n")}
                      </dd>
                    </div>
                  )}
                </dl>
              ...
              <div className="p-6">
                ...
                <div className="divide-y divide-primary/40">
                  {actingCredits.map((c) => {
                    const year =
                      c.media_type === "movie"
                        ? yearOf(c.release_date)
                        : yearOf(c.first_air_date);
                    const title = c.media_type === "movie" ? c.title : c.name;
                    return (
                      <div
                        key={`${c.media_type}-${c.id}-${c.character}-${c.job}`}
                        className="py-3 flex items-center gap-3"
                      >
                        <div className="text-tertiary/70 w-14 shrink-0 text-sm">
                          {year}
                        </div>
